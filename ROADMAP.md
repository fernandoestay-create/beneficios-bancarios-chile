...
- **"Otros beneficios" ampliado 24 → 788 verificables (10 bancos):** flips **L-32** de **Banco de Chile** (+359, `7c8c93b`), **BCI** (+172, `ca07c4b`), **Banco Security** (+80, `02920e7`), **Banco Falabella** (+61, `b405780`), **Entel** (+49) y **Tenpo** (+18, `0b851ef`), **Lider BCI** (+17, `8a1b0e3`) y **Mach** (+9, `63cf72e`). Todos: sus fuentes ya traían todas las categorías y botaban las no-restaurante (API CMS / tabs / Webflow / Drupal JSON:API / JSON embebido / páginas RSC por categoría). Trampas **L-34** manejadas; parsers con regex `%`; Mach descarta auto-promos (L-40). **Restaurantes SIEMPRE intactos** — **GATE en CADA banco: NINGÚN restaurante baseline perdido** (algún +1/+2 fue data drift real del banco, no del cambio). `beneficios.json` NUNCA tocado. Prod: BdChile 359 + BCI 172 + Security 80 + Falabella 61 + Entel 49 + Santander 21 + Tenpo 18 + Lider 17 + Mach 9 + Consorcio 2 = **788**.
- **Expansión "otros" COMPLETA — los 4 bancos restantes NO tienen descuentos % fuera de restaurantes** (verificado, incl. navegador para los SPA): **Itaú** (no-gourmet = acumulación de puntos), **Ripley** (secciones no-restaurante vacías, `haveItems:false`), **Scotiabank** (todo su catálogo son Rutas de comida; lo no-comida es su programa de puntos), **BancoEstado** (campaña caída). Usan programas de puntos/recompensas, no % en comercios.
...
- Lecciones **L-40, L-41**. Tags **`v2.2`→`v2.8`** (`cuotas-otros-audit`, `lider-otros`, `bancochile-otros`, `entel-tenpo-otros`, `security-otros`, `mach-otros`, `falabella-otros`).
...
9. **"Otros beneficios" — más bancos: ✅ COMPLETO (2026-08-05).** Se agregaron los **8 bancos que tienen descuentos % fuera de restaurantes**: Banco de Chile +359, BCI +172, Banco Security +80, Banco Falabella +61, Entel +49, Tenpo +18, Lider BCI +17, Mach +9 → **788 verificables** (10 bancos con "otros") en `/ver/beneficios`. Todos flips **L-32** con **GATE de restaurantes** (ningún baseline perdido) → `/ver` intacto. Filtro durable anti-financiamiento en el render (L-40/L-41). **Los 4 bancos restantes NO tienen "otros" (verificado, no re-investigar):** **Itaú** → sus no-gourmet son acumulación de puntos, no %; **Ripley** → sus secciones no-restaurante están vacías (`haveItems:false` en su API, verificado vía navegador); **Scotiabank** → todo su catálogo de descuentos son Rutas de comida; lo no-comida es su programa de puntos (Marketplace/Sorteos/Misiones/ScotiaPesos), no descuentos % (verificado vía navegador); **BancoEstado** → campaña caída (ni restaurantes). Estos 4 usan programas de puntos/recompensas, no % en comercios → estructuralmente no aportan.
