...
- **"Otros beneficios" ampliado 24 → 718 verificables (8 bancos):** flips **L-32** de **BCI** (+172, `ca07c4b`), **Lider BCI** (+17, `8a1b0e3`), **Banco de Chile** (+359, `7c8c93b`), **Entel** (+49) y **Tenpo** (+18, `0b851ef`) y **Banco Security** (+80, `02920e7`). Todos: sus fuentes ya traían todas las categorías y botaban las no-restaurante (API CMS / tabs de la página / Webflow CMS / Drupal JSON:API). Banco de Chile/Security: se quitó/amplió el filtro de categoría del servidor. Trampas **L-34** manejadas; parsers con regex `%` estricto. **Restaurantes SIEMPRE intactos** — **GATE en CADA banco: NINGÚN restaurante baseline perdido ni movido a "otros"** (BdChile 249, Entel 28, Tenpo 7 idénticos; Security +2 fue data drift real del banco, no del cambio). `beneficios.json` NUNCA tocado. Prod: BdChile 359 + BCI 172 + Security 80 + Entel 49 + Santander 21 + Lider 17 + Tenpo 18 + Consorcio 2 = **718**.
...
- Lecciones **L-40, L-41**. Tags **`v2.2`→`v2.6`** (`cuotas-otros-audit`, `lider-otros`, `bancochile-otros`, `entel-tenpo-otros`, `security-otros`).
...
9. **"Otros beneficios" — más bancos:** ✅ **6 bancos agregados (2026-08-04)**: BCI +172, Lider BCI +17, Banco de Chile +359, Entel +49, Tenpo +18, Banco Security +80 → **718 verificables** (8 bancos en total) en `/ver/beneficios`. Todos flips **L-32** (sus fuentes ya traían todo y lo botaban), con **GATE de restaurantes** en cada uno (ningún baseline perdido) → `/ver` intacto. Filtro durable anti-financiamiento en el render (L-40/L-41). **Pendiente:** **Mach** (su fetch usa `?catg=restaurantes` → probablemente flip fácil quitando el param); **Falabella/Itaú/Scotiabank** (fetch nuevo scoped a restaurante); **Ripley** (SPA, sus `idSection` se cargan por JS → navegador). Incremental.
...
- **Apartado "Otros beneficios" (DESPLEGADO):** `/ver/beneficios` muestra **718 beneficios verificables (8 bancos)** — **Banco de Chile 359 + BCI 172 + Security 80 + Entel 49 + Santander 21 + Lider BCI 17 + Tenpo 18 + Consorcio 2** — de `beneficios_otros.json` (`seccion="otro"`, 720 en el archivo; el render oculta los sin % real, L-40/L-41). Financiamiento/servicios/CAE/cuotas no se muestran (L-33/L-34/L-35). 6 bancos agregados hoy vía flip L-32 (fuente ya traía todo) con gate de restaurantes
