...
- **Itaú investigado y descartado:** sus beneficios no-gourmet son **acumulación de puntos**, no descuentos % → 0 aportan al apartado (el filtro `%>0` los excluye). No se agregó.
...
9. **"Otros beneficios" — más bancos:** ✅ **8 bancos agregados (2026-08-04)**: Banco de Chile +359, BCI +172, Banco Security +80, Banco Falabella +61, Entel +49, Tenpo +18, Lider BCI +17, Mach +9 → **788 verificables** (10 bancos en total) en `/ver/beneficios`. Todos flips **L-32** con **GATE de restaurantes** (ningún baseline perdido) → `/ver` intacto. Filtro durable anti-financiamiento en el render (L-40/L-41). **Pendiente (3 bancos, cada uno con su bloqueo):** **Itaú** — investigado: sus no-gourmet son **acumulación de puntos**, no descuentos %; no aporta (descartado). **Scotiabank** — SPA (React); sus "categorías" son secciones del programa (pesos/misiones/sorteos), no rubros de comercio → requiere hallar la API de datos detrás del SPA. **Ripley** — SPA; sus `idSection` se cargan por JS → inspección de red con navegador. Los 3 requieren navegador o ingeniería inversa de su API; ya no son flips.
