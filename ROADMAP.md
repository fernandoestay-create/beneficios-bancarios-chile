...
- **"Otros beneficios" ampliado 24 → 571 verificables:** flips **L-32** de **BCI** (23→195, `ca07c4b`), **Lider BCI** (+17, `8a1b0e3`) y **Banco de Chile** (+359, `7c8c93b`). BCI/Lider: sus APIs CMS ya traían todo y lo botaban. Banco de Chile: su API filtraba por categoría en el servidor → se quitó el filtro `meta.category` y se clasifica por categoría (salud, belleza, deportes, panoramas, óptica…). Lider manejó trampas **L-34** ("$100 Dcto."=monto, no 100%; cuotas excluidas); Banco de Chile ya usa regex `%` estricto (a prueba de L-34). **Restaurantes SIEMPRE intactos** (`beneficios.json` NO tocado; **GATE verificado en Banco de Chile: los 249 restaurantes quedan con IDs idénticos** → `/ver` no cambia). Verificado en prod: BdChile 359 + BCI 172 + Santander 21 + Lider 17 + Consorcio 2 = **571**.
...
- Lecciones **L-40, L-41**. Tags **`v2.2-cuotas-otros-audit`** + **`v2.3-lider-otros`** + **`v2.4-bancochile-otros`**.
...
9. **"Otros beneficios" — más bancos:** ✅ **BCI + Lider BCI + Banco de Chile agregados (2026-08-04)**: BCI 23→195 (`ca07c4b`) + Lider BCI +17 (`8a1b0e3`) + Banco de Chile +359 (`7c8c93b`) → **571 verificables** en `/ver/beneficios`. BCI/Lider = flips **L-32** (API CMS traía todo y lo botaba). Banco de Chile = se quitó su filtro `meta.category` y se clasifica por categoría (**GATE:** 249 restaurantes con IDs idénticos → `/ver` intacto). Filtro durable anti-financiamiento en el render (L-40/L-41). **Pendiente (próximos):** Ripley/Entel/Tenpo (HTML, más frágil, fetch nuevo). Falabella/Itaú/Scotiabank/Security/Mach necesitan **fetch nuevo** (su URL está scoped a restaurante). Incremental, banco por banco.
...
- **Apartado "Otros beneficios" (DESPLEGADO):** `/ver/beneficios` muestra **571 beneficios verificables** — **Banco de Chile 359 + BCI 172 + Santander 21 + Lider BCI 17 + Consorcio 2** — de `beneficios_otros.json` (`seccion="otro"`, 573 en el archivo; el render oculta los sin % real, L-40/L-41). Financiamiento/servicios/CAE/cuotas no se muestran (L-33/L-34/L-35). 5 bancos capturados (BCI/Lider = flip L-32; Banco de Chile = quitar filtro de categoría + gate de restaurantes idénticos)
