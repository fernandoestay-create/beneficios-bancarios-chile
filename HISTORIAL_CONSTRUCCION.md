...
| 13 | 2026-08-04 | **Cuotas a agosto + "Otros" ampliado 24→718 (8 bancos) + audit (v2.2→v2.6)**: cuotas re-curadas a agosto (11/14 bancos, 9 oficial-verificada), **"Otros beneficios" 24→718** — 6 bancos vía flip **L-32** (fuente ya traía todo y lo botaba): BCI +172, Lider +17, **Banco de Chile** +359 (quitado filtro `meta.category`), **Entel** +49 (tabs de la página), **Tenpo** +18 (Webflow CMS), **Banco Security** +80 (2ª pasada Drupal JSON:API). **GATE en cada banco: ningún restaurante baseline perdido** (BdChile 249, Entel 28, Tenpo 7 idénticos) → `/ver` intacto. Trampas L-34 manejadas, **filtro de calidad DURABLE en el render** (L-40/L-41), Ripley región corregida. Pendiente: Mach (flip fácil), Falabella/Itaú/Scotiabank (fetch nuevo), Ripley (SPA/navegador) |
