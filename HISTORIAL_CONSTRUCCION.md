...
| 13 | 2026-08-04 | **Cuotas a agosto + "Otros" ampliado 24→638 (7 bancos) + audit (v2.2→v2.5)**: cuotas re-curadas a agosto (11/14 bancos, 9 oficial-verificada), **"Otros beneficios" 24→638** — 5 bancos vía flip **L-32** (fuente ya traía todo y lo botaba): BCI +172, Lider +17, **Banco de Chile** +359 (quitado filtro `meta.category`), **Entel** +49 (tabs de la página), **Tenpo** +18 (Webflow CMS). **GATE en cada banco: restaurantes con IDs idénticos** (BdChile 249, Entel 28, Tenpo 7) → `/ver` intacto. Trampas L-34 manejadas, **filtro de calidad DURABLE en el render** (L-40/L-41), Ripley región corregida. Pendiente: Ripley (SPA/navegador) + 5 bancos scoped a restaurante (fetch nuevo) |
